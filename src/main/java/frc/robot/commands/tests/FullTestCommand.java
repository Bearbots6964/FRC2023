...
public class FullTestCommand extends InstantCommand {
  ...
  public FullTestCommand(
      TestArmCommand arm,
      TestClawCommand claw,
      TestDrivebaseCommand drive,
      TestTurretCommand turret) {
    m_arm = arm;
    m_claw = claw;
    m_drive = drive;
    m_turret = turret;
  ...
  @Override
  public void execute() {
    m_arm.andThen(m_claw.andThen(m_drive.andThen(m_turret))).schedule();
  }
